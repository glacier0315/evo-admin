...
import com.glacier.component.model.Result;
...
import com.glacier.evo.app.sys.convert.UserConvert;
import com.glacier.evo.app.sys.dto.UserDetailsDto;
import com.glacier.evo.app.sys.dto.user.*;
...
public class UserServiceImpl implements UserService, PageableService {
    ...
    public PageResponse<UserDto> findPage(PageRequest<UserQuery> pageRequest) {
		return findPage(pageRequest, params -> this.userConvert.toUserDto(this.userMapper.selectList(params)));
    ...
    public boolean checkUsername(UserDto userDto) {
        if (userDto != null
            ...
            return this.userMapper.checkUsernameExist(new UsernameQuery(userDto.getId(), userDto.getUsername())) > 0;
        }
        return false;
    }
    ...
    public boolean checkIdCard(UserDto userDto) {
        if (userDto != null
            ...
            return this.userMapper.checkIdCardExist(new UserIdCardQuery(userDto.getId(), userDto.getIdCard())) > 0;
        }
        return false;
    }
    ...
    public int saveUserRole(final String userId, List<String> roleIds) {
        ...
        if (StringUtils.isNotEmpty(userId)
                ...
                && !roleIds.isEmpty()) {
            // 保存用户角色关系
            return this.userRoleMapper.insertBatchRole(new One2Many<>(userId, roleIds));
        }
        return update;
    }
}
